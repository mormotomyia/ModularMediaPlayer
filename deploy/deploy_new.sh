...
oldVersion="$(az acr repository show-tags --name emsusignage --repository "$repository" | sort --version-sort | tail -n 3 | head -n 1 | sed 's/"//g; s/ //g; s/,//g ')"
...
newVersion="${oldVersion:-0.0.1-amd64}"
...
then
  ...
  docker build --cache-from emsusignage.azurecr.io/$repository:$oldVersion -f "./Dockerfile.deployment" -t "emsusignage.azurecr.io/$repository:$newVersion" "."
  echo "build with cache"
else
  docker build -f "./Dockerfile.deployment" -t "emsusignage.azurecr.io/$repository:$newVersion" "."
fi
