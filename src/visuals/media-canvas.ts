...
export interface IScreen{
    [key: string]: {"p1":{ x:number, y:number}, "p2":{ x:number, y:number},rotate:any}
}
...
export class MediaCanvas{
    startTimeout: any;
    timeout: any;
    defaultContentEndAction: Function = () => this.rootelement.style.display = "none";
    ...
    constructor(container:HTMLElement,layer:number,canvas_size:{ x:number, y:number}, screens:IScreen, debug:boolean){
        this.container = container;
        // console.log(this.container)
        this.rootelement = document.createElement('div')
        this.rootelement.classList.add('media-canvas')
        this.rootelement.id = layer.toString();
        this.rootelement.setAttribute("style", `display:none; z-index:${layer}; height:${canvas_size.y}px ; width:${canvas_size.x}px ; left:0px; top: 0px; position: absolute; overflow: hidden;`)
        
        if(debug){
            // console.log(this.rootelement.getAttribute("style"))

            this.rootelement.setAttribute("style", `${this.rootelement.getAttribute("style")};background-color:rgb(50,30,120)`)
        }
        ...
        Object.entries(screens).forEach((element:Array<any>) => {
            ...
            // {p1: { x:number, y:number}, p2: { x:number, y:number}}
                ...
                transform: rotate(${element[1].rotate!==true?element[1].rotate:"90deg"});
            ...
            if(debug){
                canvasElement.setAttribute("style",  `${canvasElement.getAttribute("style")};background-color:rgb(${element[1].p2.x%256},${element[1].p2.y%256},50)`)
            }
    ...
    private createMediaElements(canvasElement:HTMLDivElement) {
        ...
        this.mediaContainers.set(canvasElement.id,new MediaElement(canvasElement, false));
    ...
    private checkReady():void{
        ...
        if (this.swapper.every((value:IMediaElement) => {   
                    const helperVideo = value.video.find(e => e.className ===`active`)
                    const helperImage = value.image.find(e => e.className ===`active`)
                    const helperBanner = value.banners.find(e => e.className ===`active`)
    ...
    setMedias(media: Array<{element:string,type:MediaType, source: string}>):void{
        this.swapper = []

        console.log("swapping")
            media.forEach(value => {
                const {element,type,source}=value
                // if( this.mediaContainers.get(element)) console.log('Aaaaa')
    ...
    setMedia(element:string, type:MediaType ,source:string):void{
        console.log(element)

        let unused:Array<HTMLElement> = [];
        switch (type) {
            case MediaType.Video:
                const video = this.mediaContainers.get(element).video.find(e => e.className ===`active`) 
                video.getElementsByTagName('source').item(0).setAttribute('src',source)
            ...
            case MediaType.Image:
                const img = this.mediaContainers.get(element).image.find(e => e.className ===`active`)
            ...
            case MediaType.HTML:
                const banner = this.mediaContainers.get(element).banners.find(e => e.className ===`active`)
                ...
                fetch(source,{
                    ...
                    .then(data => {
                        ...
                        unused =this.mediaContainers.get(element).activeNonUsed(banner)
                        console.log(unused)
                        this.setUnusedDisplayNone(unused)
                    })
                    .catch((error) => {
