...
buildArgs='--build-arg buildCommand=build-production+spotlight' 

# oldVersion may be "" if no old version existed.
oldVersion="$(az acr repository show-tags --name emsusignage --repository "$repository" | sort --version-sort | grep spotlight |tail -n 3 | head -n 1 | sed 's/"//g; s/ //g; s/,//g ')"
...
newVersion="${oldVersion:-0.0.1.spotlight}"
